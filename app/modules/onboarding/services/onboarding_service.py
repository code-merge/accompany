...
from app.core.db.db_seed import (
...
async def provision_site(session: dict) -> AsyncGenerator[str, None]:
    ...
    msg = await initialize_schema(engine)
    yield msg
    if msg.startswith("❌"):
        yield "❌ Aborting: Schema setup failed"
        yield "done"; return

    async with AsyncSession(engine) as db:
        yield "👤 Creating system admin..."
        ok, msg = await create_system_admin_user(db, email, password)
        yield msg
        if not ok:
        ...
        ok, msg = await create_company_record(db, company, industry)
        yield msg
        if not ok:
        ...
        msg = await seed_modules(db, modules)
        yield msg
        if msg.startswith("❌"):
